...
import sys,getopt,got,datetime,codecs,time
...
def main(argv):
 ...
 python Exporter.py --username "barackobama" --maxtweets 10 --toptweets\n"""
	...
	try:
		...
		outputFile.write(';text;favouiteCount;created;id;screenName;retweetCount;mentions;hashtags;permalink;geo')
		...
		def receiveBuffer(tweets):
			count = 1
			for t in tweets:
				outputFile.write('\n%d;"%s";%d;%s;"%s";%s;%d;%s;%s;%s;%s' % (count, t.text, t.favorites, t.date.strftime("%m/%d/%Y %H:%M"), t.id, t.username, t.retweets, t.mentions, t.hashtags, t.permalink, t.geo))
				count += 1
			outputFile.flush();
			print time.time()
			print 'Saved %d tweets on file...\n' % len(tweets)
